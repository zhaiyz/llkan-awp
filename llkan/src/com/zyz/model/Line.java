...
public class Line {
	...
	public static LinkedList<Line> scan(Map map, Point a, Point b) {   
        LinkedList<Line> ll = new LinkedList<Line>();   
        for(int y = a.y; y >= 0; y--)   
            if(map.getMap()[a.x][y] == 0 && map.getMap()[b.x][y] == 0 && Match.verticalMatch(map, new Point(a.x, y), new Point(b.x, y)))   
        ...
        for(int x = a.x; x >= 0; x--)   
            if(map.getMap()[x][a.y] == 0 && map.getMap()[x][b.y] == 0 && Match.horizonMatch(map, new Point(x, a.y), new Point(x, b.y)))   
